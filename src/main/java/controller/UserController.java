...
public class UserController {
	...
	public ResponseEntity<List<UserDTO>> getAll() {
		...
		List<UserDTO> userDTOs = users.stream().map(usuarioService::toDTO).collect(Collectors.toList());
	...
	public ResponseEntity<UserDTO> getById(@PathVariable long id) {
		return repository.findById(id).map(usuarioService::toDTO).map(ResponseEntity::ok)
